...
void S21Matrix::CopyResult(S21Matrix &result) {
  int k = 0;
  int rows = this->rows_;
  int cols = this->cols_;
...
void S21Matrix::InverseAugmented(const S21Matrix &initial_matrix) {
...
void S21Matrix::CreateAugmentMatrix(const S21Matrix &initial_matrix) {
...
S21Matrix S21Matrix::InverseMatrix() {
  ...
  S21Matrix adj_matrix(rows_, 2 * cols_);
